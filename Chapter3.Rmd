...
How can we compute Sn for various values of n, say n=1,\dots,25? Do we write 25 lines of code calling compute_s_n? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of n. For-loops let us define the range that our variable takes (in our example n=1,\dots,25), then change the value and evaluate expression as you loop.
...
library(dslabs)
data(murders)
beofre_new_names <- murders$abb[nchar(murders$state) > 8]
beofre_new_names
#a <- 0
#ifelse(a > 0, 1/a, NA)
...
```{r}
#practice
#qq <- nchar(murders$state) > 8
#if(qq){
#  print(murders$abb) 
#} else{
#  print(murders$state)
#}#it doesn't works on vectors?
#a <- c(0,1,2,3,4)
#ifelse(a > 0, 1/a, NA) [1] NA 1.0000000 0.5000000 0.3333333 0.2500000
#if(a>0){
#  print(1/a)
#} else{
#  print(NA)
...
#x -> c(1, 2, -3, 4)
#y -> 1:10
#beofre_altman_plot <- plot(x+y, abs(x-y))
...
#sapply(n, function-especially "already assigned function")
```

10. Repeat exercise 8, but this time use map_dbl.
```{r}
#number10 answer: 
#map_dbl is in chapter4.11
library(tidyverse)
...
identical(S_n(25), compute_s_n(25))
#S_n <- n*(n+1)*(2*n+1)/6 is not right.
#Because class(S_n) is numeric, not function.
```
